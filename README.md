...
- **MCP tool-poisoning efficacy** (ADR 004): A weakly-worded decoy tool description failed to hijack the agent, but strengthening it with authoritative, policy-like language and an innocuous tool name succeeded, the attack strength, not just the presence of a malicious tool, determines outcome.
