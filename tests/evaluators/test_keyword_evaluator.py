...
@pytest.mark.asyncio
async def test_keyword_found_case_insensitive() -> None:
...
@pytest.mark.asyncio
async def test_keyword_found_exact_case() -> None:
...
@pytest.mark.asyncio
async def test_keyword_not_found() -> None:
...
@pytest.mark.asyncio
async def test_keyword_empty_responses() -> None:
    """No responses at all means no match."""
    evaluator = KeywordEvaluator(target_phrase="anything")
    attack = _DummyAttack()

    result = await evaluator.evaluate(attack, ["turn"], [])
